...
void productor(int tag)
{
   ...
   { 
      value = i ;
      cout << "Productor " << tag << " produce valor: " << value << endl << flush ;
      ...
      // enviar 'value'
      MPI_Ssend( &value, 1, MPI_INT, Buffer, TPRODUCTOR, MPI_COMM_WORLD );
   }
}
// ---------------------------------------------------------------------
...
void buffer()
{
   ...
   {  
      ...
      switch(rama)
      {
         case 0:        //En el caso de que reciba del productor
            MPI_Recv( &value[pos], 1, MPI_INT, MPI_ANY_SOURCE, TPRODUCTOR, MPI_COMM_WORLD, &status );
            cout << "Buffer recibe " << value[pos] << " de Productor " << endl << flush;  
            pos++;
            break;
         case 1:        //En el caso de que reciba del consumidor
            MPI_Recv( &peticion, 1, MPI_INT, MPI_ANY_SOURCE, TCONSUMIDOR, MPI_COMM_WORLD, &status );
            MPI_Ssend( &value[pos-1], 1, MPI_INT, status.MPI_SOURCE, TBUFFER, MPI_COMM_WORLD);
...
void consumidor(int tag)
{
   ...
   for (unsigned int i=0;i<ITERS/4;i++)
   {
      ...
      MPI_Ssend( &peticion, 1, MPI_INT, Buffer, TCONSUMIDOR, MPI_COMM_WORLD ); 
      
      MPI_Recv ( &value, 1,    MPI_INT, Buffer, TBUFFER, MPI_COMM_WORLD, &status );
